...
wp_lats = [[float(x) for x in sys.argv[1].split(',')]]
wp_lons = [[float(x) for x in sys.argv[2].split(',')]]
#wp_alts = []
base_alts = sys.argv[3]
top_alts = sys.argv[4]
wrf_fil = input_dir + sys.argv[7]
...
#if (np.shape(wp_lats)[0] == np.shape(wp_lons)[0] == np.size(base_alts) == np.size(top_alts)):
...
# Calculate levels to interpolate onto
   ...
   lev_interval = (float(top_alts) - float(base_alts))/100.0
   interp_levs = np.arange(float(base_alts), float(top_alts)+lev_interval, lev_interval)
...
#   if (np.size(wp_lats[i]) == np.size(wp_lons[i]) == np.size(wp_alts[i])):
   if (np.size(wp_lats[i]) == np.size(wp_lons[i])):
...
# Plot terrain at the bottom of the cross section
            ...
            ht_fill = ax.fill_between(x_ticks, 0, 100.0*((to_np(ter_cross[k])-float(base_alts))/(float(top_alts)-float(base_alts))), facecolor="black")
